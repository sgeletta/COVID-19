...
weeks = week(dates);
dth_rates = (cases1/stdth_pop)*1000;
run;
/* -------------------------------------------------------------------
   Sort data set WORK.TSTATESUM
   ------------------------------------------------------------------- */
PROC SORT
	DATA=WORK.TSTATEDTHSUMOUT(KEEP=dates dth_rates StateAbb CensRegName)
	OUT=WORK.StateDeathRates
	;
